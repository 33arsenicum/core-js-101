...
function findFirstSingleChar(str) {
  ...
  return chars.find((char) => chars.filter((c) => c === char).length === 1) || null;
}
...
function getDigitalRoot(num) {
  let digitalRoot = num;
  while (digitalRoot > 9) {
    digitalRoot = digitalRoot.toString().split('').reduce((acc, curr) => acc + parseInt(curr, 10), 0);
  }
  return digitalRoot;
}
